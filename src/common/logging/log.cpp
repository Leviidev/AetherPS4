...
void Setup(std::string_view shadps4_filename) {
    ...
    for (auto& [name, logger] : ALL_LOGGERS) {
        ...
        // Flush to disk after every single message, not just on buffer-full/exit --
        // a hard crash (SIGSEGV/SIGABRT from JIT'd guest code, corrupted native state,
        // etc.) never runs atexit/quick_exit, so anything still sitting in spdlog's
        // internal write buffer at that point is lost. This is the difference between
        // a crash log that ends right before the actual failure and one that has it.
        logger->flush_on(spdlog::level::trace);
    }
    // Backstop in case a message is somehow logged through a path that bypasses
    // flush_on (e.g. a future logger added without it): re-flush the actual sinks
    // on a timer regardless of what was just logged. Not spdlog::flush_every() --
    // that only walks spdlog's global logger registry, and ALL_LOGGERS' loggers
    // are never registered into it (they're only ever reached via this map), so
    // it would silently flush nothing.
    static std::once_flag flush_thread_started;
    std::call_once(flush_thread_started, [] {
        std::thread([] {
            while (true) {
...
void Switch(std::string_view game_filename) {
    UpdateSinks();
    // Forced to max verbosity while actively debugging, bypassing EmulatorSettings.GetLogFilter()
    // entirely: a saved config.toml from any earlier run persists its own `filter` value (see
    // EmulatorSettingsImpl::Load, emulator_settings.cpp) and would silently override
    // LogSettings::filter's C++ default (emulator_settings.h) on every subsequent launch,
    // defeating a default-only change on an app that's already been run before. Revert to
    // `UpdateLogLevels(EmulatorSettings.GetLogFilter());` once this investigation is done.
    UpdateLogLevels("*:trace");
