...
s32 MemoryManager::MapMemory(void** out_addr, VAddr virtual_addr, u64 size, MemoryProt prot,
    ...
    if (type == VMAType::Flexible) {
        ...
        while (handle != fmem_map.end() && remaining_size != 0) {
            ...
            // Tracy memory tracking breaks from merging memory areas. Disabled for now.
            // TRACK_ALLOC(out_addr, size_to_map, "VMEM");
            // Logged so an overlapping/double-carved physical backing offset between two
            // unrelated Flexible allocations -- which would look completely fine at map time on
            // both sides, and only surface much later as an inexplicable fault on one of them
            // when something perturbs the other -- is directly visible by grepping this instead
            // of inferred from a much-later crash address.
            LOG_INFO(Kernel_Vmm,
                     "MapMemory(Flexible): virtual {:#x}-{:#x} <- physical backing offset {:#x} "
                     "(size={:#x}) name='{}'",
                     current_addr, current_addr + size_to_map, new_fmem_area.base, size_to_map,
                     name);
