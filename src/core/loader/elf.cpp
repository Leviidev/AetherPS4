...
void Elf::LoadSegment(u64 virtual_addr, u64 file_offset, u64 size) {
    if (!is_self) {
        ...
        // ReadRaw's return value (how many bytes fread() actually read) was previously
        // discarded here -- a short read (unexpected EOF, transient I/O error) silently left
        // the unwritten tail of this segment's destination memory un-materialized in its
        // MAP_SHARED backing file, with no error logged anywhere. Later guest code reading
        // that tail SIGBUSes far away from here with nothing pointing back to this call.
        const auto read = m_f.ReadRaw<u8>(reinterpret_cast<u8*>(virtual_addr), size);
        if (read != size) {
            LOG_CRITICAL(Loader,
                         "LoadSegment: short read at file_offset={:#x}: expected {:#x} bytes, "
                         "got {:#x} -- destination memory at {:#x} is only partially "
                         "initialized past this point",
                         file_offset, size, read, virtual_addr);
        }
        return;
    }
    ...
    for (uint16_t i = 0; i < m_self.segment_count; i++) {
        ...
        if (seg.IsBlocked()) {
            ...
            if (file_offset >= phdr.p_offset && file_offset < phdr.p_offset + phdr.p_filesz) {
                ...
                // See the !is_self branch above for why this return value must be checked.
                const auto read = m_f.ReadRaw<u8>(reinterpret_cast<u8*>(virtual_addr), size);
                if (read != size) {
                    LOG_CRITICAL(Loader,
                                 "LoadSegment (self): short read at file_offset={:#x} "
                                 "(segment offset={:#x}): expected {:#x} bytes, got {:#x} -- "
                                 "destination memory at {:#x} is only partially initialized "
                                 "past this point",
                                 file_offset, offset, size, read, virtual_addr);
                }
                return;
            }
        }
