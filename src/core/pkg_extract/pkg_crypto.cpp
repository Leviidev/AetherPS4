...
void Crypto::RSA2048Decrypt(std::span<uint8_t, 32> out_key,
                            std::span<const uint8_t, 256> ciphertext,
                            bool is_dk3) {
    // Real PKCS#1 v1.5 decrypt via LibreSSL's BIGNUM modexp (see
    // pkg_rsa_native.cpp). The hand-rolled bigint CRT decrypt above this
    // function (rsa_pkcs1_v15_decrypt_crt) is kept only as a reference of the
    // algorithm; it's schoolbook modexp and was too slow for 2048-bit CRT on
    // Android, which is why that platform used a JNI/javax.crypto bridge
    // instead. LibreSSL's BN_mod_exp is fast enough to call directly here.
    if (bachata_pkg_rsa_decrypt(ciphertext.data(), out_key.data(), is_dk3 ? 1 : 0) != 0) {
        throw std::runtime_error("RSA decrypt failed (native PKCS1)");
    }
}
