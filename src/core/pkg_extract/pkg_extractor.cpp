// PKG stream extract, ported from the Bachata Android runtime's JNI module
// (adapted from shadPS4 PKG::Extract / ExtractFiles) for use as a native
// desktop command-line tool. Logging and crypto backends are swapped for
// their desktop equivalents (see pkg_rsa_native.cpp); the extraction/crypto
// algorithm itself is unchanged from the Android version.
...
#define LOGI(...) std::fprintf(stderr, "[BachataPkg] " __VA_ARGS__), std::fprintf(stderr, "\n")
#define LOGE(...) std::fprintf(stderr, "[BachataPkg] " __VA_ARGS__), std::fprintf(stderr, "\n")
