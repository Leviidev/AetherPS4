...
static void* RunThread(void* arg) {
    ...
    if (AetherPS4::GuestCpu::IsGuestFunctionAddress(
        ...
        // (e.g. via an indirect table lookup), landing mid-instruction of unrelated code.
        // Executing that runs the guest CPU off into undefined register/memory state, which
        // used to hard-abort the whole emulator (RunGuestFunctionOrAbort). Call the
        // non-aborting API directly instead so a single malformed thread just fails to start
        // (this thread never runs, but the rest of the game keeps going) rather than taking
        // the whole process down.
        auto* const linker = Common::Singleton<Core::Linker>::Instance();
        const auto guest_result = linker->RunGuestFunction(
            reinterpret_cast<VAddr>(curthread->start_routine), start_arguments,
            reinterpret_cast<VAddr>(stack));
        if (const auto* failure = std::get_if<Core::GuestExecutionFailure>(&guest_result)) {
            LOG_ERROR(Kernel_Pthread,
                      "Guest thread '{}' start routine {:#x} failed to execute (stage {}, "
                      "error {}); thread will not run.",
                      curthread->name, reinterpret_cast<VAddr>(curthread->start_routine),
                      static_cast<int>(failure->Stage), failure->Error);
            ret = nullptr;
        } else {
            ret = reinterpret_cast<void*>(std::get<u64>(guest_result));
        }
    } else {
