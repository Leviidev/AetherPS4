...
s32 PS4_SYSV_ABI sceVoiceCreatePort(u32 user_id, void* param, u32* out_port_id) {
    LOG_ERROR(Lib_Voice, "(STUBBED) called user_id={}", user_id);
    if (out_port_id) {
        *out_port_id = 1;
    }
    return ORBIS_OK;
...
s32 PS4_SYSV_ABI sceVoiceGetPortInfo(u32 port_id, OrbisVoicePortInfo* info) {
    LOG_ERROR(Lib_Voice, "(STUBBED) called port_id={}", port_id);
    if (info) {
        std::memset(info, 0, sizeof(OrbisVoicePortInfo));
        info->frame_size = 1;
    }
    return ORBIS_OK;
}
...
s32 PS4_SYSV_ABI sceVoiceReadFromOPort(u32 port_id, void* buf, u32 size, u32* read_bytes) {
    LOG_ERROR(Lib_Voice, "(STUBBED) called port_id={}", port_id);
    if (read_bytes) {
        *read_bytes = 0;
    }
...
s32 PS4_SYSV_ABI sceVoiceWriteToIPort(u32 port_id, const void* buf, u32 size, u32* written_bytes) {
    LOG_ERROR(Lib_Voice, "(STUBBED) called port_id={}", port_id);
    if (written_bytes) {
        *written_bytes = size;
    }
