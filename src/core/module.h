...
public:
    ...
    bool IsSystemLib() {
        ...
        const bool result = file.string().starts_with(system_path.string().c_str());
        // Temporary while diagnosing a VMAType::Code vs Flexible mismatch on system libraries
        // (module.cpp:171 picks the VMAType based on this result) -- prints the exact two
        // strings being compared so an iOS-sandbox path mismatch (e.g. a /var vs /private/var
        // symlink-resolution difference between how `file` and `system_path` each got built)
        // is directly visible rather than inferred.
        LOG_INFO(Core_Linker, "IsSystemLib({}): file='{}' system_path='{}' -> {}", name,
                 file.string(), system_path.string(), result);
        if (result) {
            return true;
        }
        return false;
