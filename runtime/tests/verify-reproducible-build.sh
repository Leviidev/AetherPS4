...
extract_hashes() {
  ...
  unzip -q -p "$zip" "bin/shadps4" > "$tmp_dir/shadps4"
  ...
  unzip -q -p "$zip" "host/box64" > "$tmp_dir/box64"

  echo "bin/shadps4:                $(sha256sum "$tmp_dir/shadps4" | cut -d' ' -f1)"
  ...
  echo "host/box64:                 $(sha256sum "$tmp_dir/box64" | cut -d' ' -f1)"
